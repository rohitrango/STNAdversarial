...
def convert_tensor(key, d):
    d[key] = torch.from_numpy(np.array(d[key], np.float32, copy=False)).permute(2, 0, 1).contiguous()
    return d
...
def load_class_images(d):
    if d['class'] not in MINIIMAGENET_CACHE:
        ...
        class_images = sorted(glob.glob(os.path.join(image_dir, '{}*'.format(classname))))
...
def load(opt, splits):
    # miniimagenet, splits, split name
