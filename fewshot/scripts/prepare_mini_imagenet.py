...
for c in background_classes:
    mkdir(DATA_PATH + '/miniImageNet/images_background/{}/'.format(c))

for c in evaluation_classes:
    mkdir(DATA_PATH + '/miniImageNet/images_evaluation/{}/'.format(c))
...
for root, _, files in os.walk(DATA_PATH + '/miniImageNet/images'):
    for f in tqdm(files, total=600*100):
        if f.endswith('.jpg'):
            ...
            src = '{}/{}'.format(root, f)
            dst = DATA_PATH + '/miniImageNet/{}/{}/{}'.format(subset_folder, class_name, image_name)
            shutil.copy(src, dst)
